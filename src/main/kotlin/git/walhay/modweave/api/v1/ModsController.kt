...
class ModsController @Autowired constructor(private val modRepository: ModRepository, private val modService: ModService,
  ...
  @PostMapping fun uploadMod(@ModelAttribute modUploadForm: ModUploadDTO) {
    ...
    @GetMapping("/{modId}")
    fun downloadFile(@PathVariable modId: Long, @RequestParam(required = true) version: String, @RequestParam(required = true) filename: String): String {
        val modName = modRepository.findById(modId).get().name
        val fullname = "${modName}/${version}/${filename}"
        return minioClient.getPresignedObjectUrl(GetPresignedObjectUrlArgs.builder().bucket("mods").method(Method.GET).`object`(fullname).expiry(60 * 10).build())
    }
}
